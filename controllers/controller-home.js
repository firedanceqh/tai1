...
module.exports.quannam = function (req, res) {
    Product.find({ cateProduct: 'nam' }, function (err, data) {
        if (err) {
            res.json({ "kq": 0, "errMsg": err });
        } else {
            Session.findOne({ _id: req.session.views }, function (err, data1) {
                if (err) {
                    ...
                    res.render('./paperuser/nam', { listproduct: data, listdata: data1 })
                }
            });
        }
    })
...
module.exports.aonam = function (req, res) {
    Product.find({ cateProduct: 'nu' }, function (err, data) {
        if (err) {
            res.json({ "kq": 0, "errMsg": err });
        } else {
            Session.findOne({ _id: req.session.views }, function (err, data1){
                if (err) {
                    ...
                    res.render('./paperuser/nu', { listproduct: data, listdata:data1 });
                }
            })
        }
    })
